"""
DICOM -> NPY preprocessing for LIDC-IDRI, adapted from the official LAND
repository (aolivtous/LAND_3DChestCT), src/utils/preproc_lidc_npy.py.
Apache License 2.0. Copyright 2026 Eurecat, Centre Tecnologic de Catalunya.
See reference_official_repo/LICENSE and reference_official_repo/NOTICE.

CHANGES FROM ORIGINAL (per Apache 2.0 sec. 4(b)):
1. Import path: `from .utils_lidc3D import ...` now points at this repo's
   local `src/data/utils_lidc3D.py`, which is a minimal extraction of just
   the 3 functions this file needs (see that file's header) rather than
   the official repo's full utils file, to avoid a false dependency on
   this project's not-yet-built VAE/U-Net modules (Phase 4+).
2. `rasterio` import moved from module-level to inside the one function
   that uses it (`save_ct_npy_as_image_seq`, `.tif` branch only), so the
   core normalize/resample/crop pipeline doesn't require installing
   rasterio when `--png_dir` isn't used. No logic change to any function.
3. `preprocess_dicom`: patient ID extraction changed from
   `dicom_dir_path.split("/")[-1]` to `os.path.basename(os.path.normpath(...))`
   — the original assumes Unix-style forward-slash paths and silently
   breaks on Windows (a path like C:/.../LIDC-IDRI-0078 would not split correctly).
   This is a real logic change, not cosmetic — worth knowing if diffing
   output against a Linux-run copy of the original.
4. This docstring and the inline "ADAPTED" comment below are the only
   other additions. All normalization/mask math and the rest of
   `preprocess_dicom` are unchanged from the original — this is
   Decision 0011's verified spec (patch/resample/crop settings,
   `normalize_ct_vol_wdm`, texture handling), reproduced faithfully,
   not reinterpreted.

Run from repo root (env `land-ct-v2` active):
    python -m src.data.preprocessing --dicom_dir <path> --npy_dir data/processed --normalize --resample --central_crop --sample_idx 1
"""
import os
import argparse
import logging
...
def save_ct_npy_as_image_seq(npy_path, out_dir, extension="png", prefix="ct"):
    ...
    if extension == "tif":
        # ADAPTED: rasterio import moved here (module-level in the original)
        # so it's only required if --png_dir/.tif output is actually used.
        import rasterio
        profile = {
            'driver': 'GTiff',
            'interleave': 'band',
...
def normalize_ct_vol_wdm(ct_array, min_hu_val=-1000, max_hu_val=2500):
    ...
    # This is the function actually used in training (Decision 0011) — clips
    # at the 99.9th percentile and does per-volume min-max, NOT a fixed
    # [min_hu_val, max_hu_val] range despite the argument names.
...
def preprocess_dicom(dicom_dir_path, npy_dir_path,
    ...
    patient_id = os.path.basename(os.path.normpath(dicom_dir_path))
    ...
    for nod_idx, anns in enumerate(nods):
        cmask, cbbox, _ = consensus(anns, clevel=clevel)
        texture = np.array([ann.texture for ann in anns]).mean()
        cmask = cmask.astype(np.float32)
        mask_vals = np.maximum(
            mask_vol[cbbox[0].start:cbbox[0].stop, cbbox[1].start:cbbox[1].stop, cbbox[2].start:cbbox[2].stop],
            cmask * texture)
    ...
    if resample:
        zoom_factors = [scan.pixel_spacing / target_res, scan.pixel_spacing / target_res,
                         scan.slice_spacing / target_res]
        resampled_vol = zoom(vol, zoom_factors, order=3, mode='nearest')
        resampled_mask_vol = zoom(mask_vol, zoom_factors, order=0, mode='nearest')
    else:
        resampled_vol = vol
