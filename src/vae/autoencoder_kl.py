...
# with modifications to support training on mid-range GPUs (<20GB)
...
class MaisiResBlock_Encoder(nn.Module):
    """
...
class MaisiResBlock_Decoder(nn.Module):
    """
    Residual block consisting of a cascade of 2 convolutions + activation + normalisation block, and a
    residual connection between input and output.
...
class MaisiDecoder(nn.Module):
    ...
    def __init__(
        ...
        if verbose:
            print("ReducedMaisi decoder info levels:")
            print(reversed_block_out_channels)
        ...
        for i in range(len(reversed_block_out_channels)):
            ...
            for block_num in range(reversed_num_res_blocks[i]):

                if block_num == reversed_num_res_blocks[i]- 1 and i < len(reversed_block_out_channels) - 1:
                    block_out_ch = reversed_block_out_channels[i + 1]
                
                blocks.append(
                    MaisiResBlock_Decoder(
                        spatial_dims=spatial_dims,
...
class AutoencoderKlReducedMaisi(AutoencoderKL, ModelMixin, ConfigMixin):
    """
    AutoencoderKL with custom MaisiEncoder and MaisiDecoder to reduce GPU memory and make Encoder and Decoder more similar in memory usage.
    ...
    def __init__(
        ...
        in_channels: int = 1,
        out_channels: int = 1,
        num_res_blocks: Sequence[int] = [1,1,1],
